 ...
 * is independent of the string rendered inside.
 *
 ...
 *
 * `TIMER_BOX_MIN_W` matches the pill's natural width for a `mm:ss` value at the
 ...
 * already is — never smaller, never larger. `COUNT_BOX_MIN_W` holds the longest
 * count (`784/784`), so the box never grows as pieces are placed.
 */
const TIMER_BOX_MIN_W = 144;
const COUNT_BOX_MIN_W = 116;
 ...
 * right. Row two: **Hint, Edges, Preview, Pause** grouped on the right, sitting
 * under the count/timer pair. Every control keeps the 1.2× scaling from the UI
 * pass — only the arrangement changed.
 */
      ...
      <View style={styles.headerRow} testID="game-header-top">
        ...
        <View style={styles.headerGroup}>
          <PopSurface
            ...
            radius={radii.pill}
            contentStyle={[styles.infoBox, styles.countBox]}
            testID="count-box"
          >
          ...
          <PopSurface
            fill={theme.colors.surface}
            radius={radii.pill}
            contentStyle={[styles.infoBox, styles.timerBox]}
            testID="timer-box"
          >
...
const useStyles = createThemedStyles((theme) =>
  StyleSheet.create({
    ...
    timerBox: {
      minWidth: TIMER_BOX_MIN_W,
    },
    countBox: {
      minWidth: COUNT_BOX_MIN_W,
    },
    clock: {
      ...typography.heading,
      fontSize: 16 * HEADER_SCALE,
